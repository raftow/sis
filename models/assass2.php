...
class Assass2 extends SisObject
{
    public static function fromExcel($lang = "ar", $params = [])
    {
        ...
        $isAssass1Only = array_flip(['ISASSASS1', 'TARGETSCIENTIFICDEGREEID', 'GRANTEDSCIENTIFICDEGREEID', 'COUNTRYID', 'SUMMERSEMESTERREGISTRATIONSTATUS']);    
        ...
        for ($page = $pageStart; $page <= $pageEnd; $page++) {
            ...
            foreach ($my_data as $row => $my_row) {
                ...
                list($errors, $sql_line) = AfwSqlHelper::oracleSqlInsertOrUpdate("STUDENTS.ACADEMICDETAILS", $tableColsArr["STUDENTS.ACADEMICDETAILS"], $my_row, $isInTablePK["STUDENTS.ACADEMICDETAILS"], $isScalar, $isNoEmptyString, $isDate, $isDatetime, $isMandatory, 'MM/DD/YYYY HH24:MI', 'YYYY-MM-DD');
                if(count($errors)==0) {
                    if ($nb_rows < 2) $sql_examples[] = $sql_line;
                    $sql .= $sql_line . "\n\t commit;\n";
                    list($errors, $sql_line) = AfwSqlHelper::oracleSqlInsertOrUpdate("STUDENTS.PERSONALINFO", $tableColsArr["STUDENTS.PERSONALINFO"], $my_row, $isInTablePK["STUDENTS.PERSONALINFO"], $isScalar, $isNoEmptyString, $isDate, $isDatetime, $isMandatory, 'MM/DD/YYYY HH24:MI', 'YYYY-MM-DD');
                    if(count($errors)==0) {
                        if ($nb_rows < 2) $sql_examples[] = $sql_line;
                        $sql .= $sql_line . "\n\t commit;\n";
                        $nb_rows++;
